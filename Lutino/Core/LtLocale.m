...
@synthesize name;
@synthesize smallFlag;
@synthesize mediumFlag;
@synthesize largeFlag;

static NSMutableDictionary *s_LocaleNames;

+(void)initialize {
    s_LocaleNames = [NSMutableDictionary new];
    [self loadLocales];
}

- (id)initWithValues:(NSString *)locale name:(NSString *)localeName
{
    self.localeCode = locale;
    self.languageCode = [self convertLocaleToLanguageCode:locale];
    self.name = localeName;
    
    NSString *countryCode = [[self convertLocaleToCountryCode:locale] lowercaseString];
    
    // self.smallFlag = [UIImage imageNamed:[countryCode stringByAppendingString:@".16x16.png"]]; // No longer used
    self.mediumFlag = [UIImage imageNamed:[countryCode stringByAppendingString:@".24x24.png"]];
    self.largeFlag = [UIImage imageNamed:[countryCode stringByAppendingString:@".32x32.png"]];
...
{
    NSString *localeName = [s_LocaleNames objectForKey:locale];
    return [self initWithValues:locale name:localeName];
}

-(NSString *)convertLocaleToLanguageCode:(NSString *)locale
{
    NSString *language;
    
    NSRange rangeOfHyphen = [locale rangeOfString:@"-"];
    
    if (rangeOfHyphen.location != NSNotFound)
    {
        language = [locale substringToIndex:rangeOfHyphen.location];
    }
    else
    {
        language = locale;
...
-(NSString *)convertLocaleToCountryCode:(NSString *)locale
{
    NSString *country;
    ...
    if (rangeOfHyphen.location != NSNotFound)
    {
        country = [locale substringFromIndex:NSMaxRange(rangeOfHyphen)];
    }
    else
    {
        country = @"";
...
/*
 ...
 English (AUS)	en_AU	8 kHz, 16 kHz
 English (UK)	en_GB	8 kHz, 16 kHz
 English (US)	en_US	8 kHz, 16 kHz
 Arabic (Egypt)	ar_EG	8 kHz, 16 kHz
 Arabic (Saudi)	ar_SA	8 kHz, 16 kHz
 Arabic (UAE)	ar_AE	8 kHz, 16 kHz
 Cantonese Chinese	zh_HK	8 kHz, 16 kHz
 Catalan (Spain)	ca_ES	8 kHz, 16 kHz
 Croatian	hr_HR	8 kHz, 16 kHz
 Czech	cs_CZ	8 kHz, 16 kHz
 Danish	da_DK	8 kHz, 16 kHz
 Dutch	nl_NL	8 kHz, 16 kHz
 Finnish	fi_FI	8 kHz, 16 kHz
 French (CAN)	fr_CA	8 kHz, 16 kHz
 French (EU)	fr_FR	8 kHz, 16 kHz
 German	de_DE	8 kHz, 16 kHz
 Greek	el_GR	8 kHz, 16 kHz
 Hebrew	he_IL	8 kHz, 16 kHz
 Hungarian	hu_HU	8 kHz, 16 kHz
 Indonesian	id_ID	8 kHz, 16 kHz
 Italian	it_IT	8 kHz, 16 kHz
 Japanese	ja_JP	8 kHz, 16 kHz
 Korean	ko_KR	8 kHz, 16 kHz
 Malay	ms_MY	8 kHz, 16 kHz
 Mandarin Chinese	cn_MA	8 kHz, 16 kHz
 Taiwanese Mandarin	zh_TW	8 kHz, 16 kHz
 Norwegian	no_NO	8 kHz, 16 kHz
 Polish	pl_PL	8 kHz, 16 kHz
 Portuguese (BR)	pt_BR	8 kHz, 16 kHz
 Portuguese (EU)	pt_PT	8 kHz, 16 kHz
 Romanian	ro_RO	8 kHz, 16 kHz
 Russian	ru_RU	8 kHz, 16 kHz
 Slovak	sk_SK	8 kHz, 16 kHz
 Spanish (EU)	es_ES	8 kHz, 16 kHz
 Spanish (MX)	es_MX	8 kHz, 16 kHz
 Spanish (US)	es_US	8 kHz, 16 kHz
 Swedish	sv_SE	8 kHz, 16 kHz
 Thai	th_TH	8 kHz, 16 kHz
 Turkish	tr_TR	8 kHz, 16 kHz
 Ukranian	uk_UA	8 kHz, 16 kHz
 Vietnamese	vi_VN	8 kHz, 16 kHz
...
- (id)initWithExplicitValues:(NSString *)locale languageCode:(NSString *)languageIsoCode name:(NSString *)localeName smallFlag:(NSString *)smallFlagFileName mediumFlag:(NSString *)mediumFlagFileName largeFlag:(NSString *)largeFlagFileName
{
    self.localeCode = locale;
    self.languageCode = languageIsoCode;
    self.name = localeName;
    self.smallFlag = [UIImage imageNamed:smallFlagFileName];
    self.mediumFlag = [UIImage imageNamed:mediumFlagFileName];
    self.largeFlag = [UIImage imageNamed:largeFlagFileName];
...
+(void)loadLocales
{
    NSManagedObjectContext *context = LtContext.instance.managedObjectContext;
    NSError *error;
    
    NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription *entity = [NSEntityDescription
                                   entityForName:@"LtLocaleData" inManagedObjectContext:context];
    [fetchRequest setEntity:entity];
    
    NSArray *fetchedObjects = [context executeFetchRequest:fetchRequest error:&error];
    
    for (LtLocaleData *info in fetchedObjects) {
        [s_LocaleNames setObject:info.name forKey:info.locale];
    }
}
